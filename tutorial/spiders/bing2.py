...
class PhysSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for list in lists:
            ...
            if "academic/profile" in temp_url:
    ...
    def parse_fullInfo(self,response):
        ...
        # temp_url2 = response.url + "&first=0&count=50&rt=2"
        ref_url = temp_url.replace("profile","papers")
        request = scrapy.Request(url=ref_url, callback=self.par_ref, headers=headers, meta={'item': copy.deepcopy(item)}, dont_filter=True)
        # request2 = scrapy.Request(url=ref_url, callback=self.par_url, headers=headers, meta={'item': copy.deepcopy(item)}, dont_filter=True)
        yield request
        # yield request2
    ...
    def par_ref(self,response):
        ...
        for author in authors:
            ...
            urllist.append(url)
            request = scrapy.Request(url=url, callback=self.parse_info, headers=headers, meta={'item': item,'author':author,'info':authorInfo,'z':z}, dont_filter=True)
            yield request
